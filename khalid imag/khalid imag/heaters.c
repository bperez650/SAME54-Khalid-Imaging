...
/*Global variables */	
int RTD_array[8] = {0,0,0,0,0,0,0,0};
int *RTD_array_ptr; 
int setpoint_array[8] = {0,0,0,0,0,0,0,0};

/* Setup ports for heaters */
...
/* Check each RTD values and store in array */
void check_RTDs(void){
	volatile int RTD = 0;
	...
	while(RTD < 1){
		switch(RTD){
			...
			temp = ADC1->RESULT.reg;
			temp = temp/255*3.3;
			RTD_array[RTD] = (temp-0.66)/0.0132;
			break;
			...
			ADC1->SWTRIG.reg = 1;	//trigger ADC to start conversion
			while(ADC1->INTFLAG.bit.RESRDY == 0){}
			RTD_array[RTD] = (((ADC1->RESULT.reg)*50)-50);
			break;
			...
			RTD_array[RTD] = (((ADC0->RESULT.reg)*50)-50);
			break;
...
/* Compare RTD & setpoint values and turn on heaters if needed */
void check_temps(void){
	Port *por = PORT;
	PortGroup *porA = &(por->Group[0]);
	PortGroup *porB = &(por->Group[1]);
	...
	volatile int i = 0;	//counter
	 while(i < 1){
		switch(i){
			...
			case 0:	//heater0
			if(RTD_array[i] < setpoint_array[i]){
				porC->OUTSET.reg = PORT_PC30; 
			}
			else{porC->OUTCLR.reg = PORT_PC30;}
			break;
...
/* Changes heater setpoints */
void heater_setpoint(void){
	volatile int temp_zone;
	volatile int temp_value;
	if((*(terminal_input_array_ptr+1) >= 48) && (*(terminal_input_array_ptr+1) <= 57)){	 //looking for number keys only
		temp_zone = (*(terminal_input_array_ptr+1) - 48);	//get zone for setpoint
	}
	
	if((*(terminal_input_array_ptr+2) >= 48) && (*(terminal_input_array_ptr+2) <= 57)){	//looking for number keys only
		if((*(terminal_input_array_ptr+3) >= 48) && (*(terminal_input_array_ptr+3) <= 57)){
			if((*(terminal_input_array_ptr+4) >= 48) && (*(terminal_input_array_ptr+4) <= 57)){	
				temp_value = (*(terminal_input_array_ptr+2) - 48) * 100;	//get setpoint value
				temp_value += (*(terminal_input_array_ptr+3) - 48) * 10;
				temp_value += *(terminal_input_array_ptr+4) - 48;
			}
		}
	}
	setpoint_array[temp_zone] = temp_value;
